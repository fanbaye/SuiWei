...
@interface YZYRefreshTableFooterView (Private)
- (void)setState:(YZYPullRefreshState)aState;
@end

@implementation YZYRefreshTableFooterView
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		[self setState:YZYOPullRefreshNormal];
...
- (void)refreshLastUpdatedDate {
	...
	if ([_delegate respondsToSelector:@selector(yzyRefreshTableFooterDataSourceLastUpdated:)]) {
		
		NSDate *date = [_delegate yzyRefreshTableFooterDataSourceLastUpdated:self];
		...
		[[NSUserDefaults standardUserDefaults] setObject:_lastUpdatedLabel.text forKey:@"YZYRefreshTableView_LastRefresh"];
...
- (void)setState:(YZYPullRefreshState)aState{
	...
	switch (aState) {
		case YZYOPullRefreshPulling:
			...
			break;
		case YZYOPullRefreshNormal:
			
			if (_state == YZYOPullRefreshPulling) {
				[CATransaction begin];
				[CATransaction setAnimationDuration:FLIP_ANIMATION_DURATION];
...
//		offset = MIN(offset, 60);
		...
		BOOL _loading = NO;
		if ([_delegate respondsToSelector:@selector(yzyRefreshTableFooterDataSourceIsLoading:)]) {
			_loading = [_delegate yzyRefreshTableFooterDataSourceIsLoading:self];
		}
		
		if (_state == YZYOPullRefreshPulling && scrollView.contentOffset.y + (scrollView.frame.size.height) < scrollView.contentSize.height + RefreshViewHight && scrollView.contentOffset.y > 0.0f && !_loading) {
			[self setState:YZYOPullRefreshNormal];
		} else if (_state == YZYOPullRefreshNormal && scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight  && !_loading) {
			[self setState:YZYOPullRefreshPulling];
		}
...
- (void)yzyRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight && !_loading) {
		...
		if ([_delegate respondsToSelector:@selector(yzyRefreshTableFooterDidTriggerRefresh:)]) {
			[_delegate yzyRefreshTableFooterDidTriggerRefresh:self];
		}
		
		[self setState:YZYOPullRefreshLoading];
